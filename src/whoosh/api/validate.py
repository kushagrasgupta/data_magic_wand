...
def validate_frame(
    ...
    *,
    ...
    if field_count is not None and field_count != len(columns):
        bad = lf.with_columns(pl.lit(f"field_count expected {field_count}, found {len(columns)}").alias("_whoosh_errors"))
    ...
    if not checks:
        ...
        empty_bad = lf.head(0)
        if append_errors:
            empty_bad = empty_bad.with_columns(pl.lit("").alias("_whoosh_errors"))
        return ValidationResult(good=lf, bad=empty_bad, total_rows=total, failed_rows=0)

    ok_expr = pl.all_horizontal(checks).alias("__whoosh_ok")
    framed = lf.with_columns(ok_expr)

    good = framed.filter(pl.col("__whoosh_ok")).drop("__whoosh_ok")
    bad = framed.filter(~pl.col("__whoosh_ok")).drop("__whoosh_ok")
    if append_errors:
        bad = bad.with_columns(pl.concat_str(error_exprs, separator="; ").alias("_whoosh_errors"))

    total = int(framed.select(pl.len()).collect().item())
    failed = int(framed.filter(~pl.col("__whoosh_ok")).select(pl.len()).collect().item())
